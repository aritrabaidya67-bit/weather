...
from ..services import ALERT_RULES, AlertService, DeviceService, RiskService, get_ollama_client, runner, scheduler
...
def status(session: SessionDep) -> dict[str, Any]:
    ...
    if settings.simulation_mode:
        notes.append(
            "SIMULATION MODE: readings are generated by the built-in simulator, not by physical hardware."
        )
    reading_stale = bool(
...
@router.get("/system/workers", summary="Background worker and simulation status")
def workers() -> dict[str, Any]:
    return {
        "background": scheduler.status(),
        "simulation": runner.status(),
        "realtime_subscribers": bus.subscriber_count,
