...
import { AlertTriangle, Database, FlaskConical, Info, Loader2, Radio, TrendingUp } from "lucide-react";
...
/** Data-provenance badge: the UI never presents simulated or predicted values as live facts. */
export function DataBadge({
  ...
  const isSimulated = simulated ?? source === "simulation";
  const label = isSimulated ? "SIMULATED DATA" : predicted ? "PREDICTED" : dataSourceLabel(source);
  const Icon = isSimulated ? FlaskConical : predicted ? TrendingUp : source === "arduino" ? Radio : Database;
  const severity = isSimulated ? "watch" : predicted ? "info" : source === "arduino" ? "good" : "unknown";
  return (
    <span
      className={classNames(
      ...
      title={
        isSimulated
          ? "Values are generated by the simulator, not measured by physical hardware."
          : predicted
